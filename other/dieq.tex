...
\textbf{Пример:}
\par $y' - y \cdot ctgx - \sin(x) = 0$
\begin{enumerate}
    \item Убедиться, что это действительно НЛДУ1П.
        \par $y' + (-ctgx) \cdot y = \sin(x)$
    ...
    \item Эту штуку (выражение справа) подставить в исходное ур-е.
        ...
        \par $(C(x)\sin(x))' - C(x)\sin(x) \cdot ctgx - \sin(x) = 0$
...
\begin{enumerate}
    ...
    \item Заменить $y = uv$, где $u = u(x)$, $v = v(x)$ -- пока неизвестные функции.
        \par $(uv)' + (tgx)(uv) = \frac{1}{\cos(x)}$
        \par $u'v + uv' + uv \cdot tgx = \frac{1}{\cos(x)}$
    \item Вынести $u$ за скобки.
        \par $u'v + v(v' + vtgx) = \frac{1}{\cos(x)}$
    \item Найти какую-то конкретную функцию $v \neq 0$, которая обнуляет выражение в скобках.
        \par \textit{Решим $v' + vtgx = 0$}
        \par $[...]$
        \par $v = C_{1}\cos(x)$
